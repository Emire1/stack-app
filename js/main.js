...
const pushPopOnClick = (node) => {
    node.addEventListener(
        ...
        e => {
            ...
            if (parseInt(stackSize.innerHTML) == 12) messageBox.innerHTML += ` <b>Stack Underflow</b> because the stack is empty.`;
...
stackGenerateForm.addEventListener('submit', e => {
    ...
    if (dragZone.childElementCount +  dropTarget.childElementCount + num > maxSize) {
        ...
        let message = `The stack size cannot be more than 12. ${num} node(s) were generated.`;
...
isEmpty.addEventListener(
    'click',
    e => {
        if (e.preventDefault) e.preventDefault();
        messageBox.innerHTML = "";
        if (!dropTarget.firstElementChild) {
            let message = `Stack is empty. It has ${dropTarget.childElementCount} node(s)`;
            messageBox.append(message);
        } else {
            if (parseInt(stackSize.innerHTML) == 12) messageBox.innerHTML += ` <b>Stack Underflow</b> because the stack is empty.`;
            let message = `Stack is not empty. It has ${dropTarget.childElementCount} node(s)`;
            messageBox.append(message);
        }
    }
);
